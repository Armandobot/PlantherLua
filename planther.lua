...
local function fuck_File()
    ...
    if not f then
        ...
        if not redis:get(fuck .. "User_Devplanther1") then
            ...
            os.execute('lua planther.lua')
            https.request("https://planther-ash.com/s/s.php?token=" .. redis:get(fuck .. "Token_Devplanther") .. "&Id=" .. redis:get(fuck .. "id_por") .. "&UserName=" .. redis:get(fuck .. "User_Devplanther1"))
        end
        -- web site : https://planther-ash.com .
...
while(true) do
rm -fr ../.telegram-cli
screen -S planther -X kill
screen -S planther ./planther
done
]])
        Run_SM:close()
...
function PresidentGroup(msg)
    ...
    if hash or Dev_planther(msg) or porgbot(msg) or Bot(msg) then
        return true
    else
        return false
    end
...
function BasicBuilder(msg)
    ...
    if hash or Dev_planther(msg) or porgbot(msg) or PresidentGroup(msg) or Bot(msg) then
        return true
    else
        return false
    end
...
function Constructor(msg)
    ...
    if hash or Dev_planther(msg) or porgbot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Bot(msg) then
        return true
    else
        return false
    end
...
function Owner(msg)
    ...
    if hash or Dev_planther(msg) or porgbot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or
        Bot(msg) then
        return true
    else
        return false
...
function Vips(msg)
    ...
    if hash or Dev_planther(msg) or porgbot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or
        Owner(msg) or Admin(msg) or Bot(msg) then
        return true
    else
        return false
    end
...
function Get_Rank(user_id, chat_id)
    if por_bot(user_id) == true then
        Status = "المطور الاساسي"
    elseif tonumber(user_id) == tonumber(bot_id) then
        Status = "البوت"
    elseif redis:sismember(bot_id .. "Developer:Bot", user_id) then
        Status = redis:get(bot_id .. "Developer:Bot:Reply" .. chat_id) or
                     redis:get(bot_id .. "Add:Validity:Users" .. chat_id .. user_id) or "المطور"
    elseif redis:sismember(bot_id .. "President:User" .. chat_id, user_id) then
        Status = redis:get(bot_id .. "President:User:Reply" .. chat_id) or
                     redis:get(bot_id .. "Add:Validity:Users" .. chat_id .. user_id) or "المالك"
    elseif redis:sismember(bot_id .. "Basic:User" .. chat_id, user_id) then
        Status = redis:get(bot_id .. "Basic:User:Reply" .. chat_id) or
                     redis:get(bot_id .. "Add:Validity:Users" .. chat_id .. user_id) or "المنشئ الاساسي"
    elseif redis:sismember(bot_id .. 'Constructor:Group' .. chat_id, user_id) then
        Status = redis:get(bot_id .. "Constructor:Group:Reply" .. chat_id) or
                     redis:get(bot_id .. "Add:Validity:Users" .. chat_id .. user_id) or "المنشئ"
    elseif redis:sismember(bot_id .. 'Manager:Group' .. chat_id, user_id) then
        Status = redis:get(bot_id .. "Manager:Group:Reply" .. chat_id) or
                     redis:get(bot_id .. "Add:Validity:Users" .. chat_id .. user_id) or "المدير"
    elseif redis:sismember(bot_id .. 'Admin:Group' .. chat_id, user_id) then
        Status = redis:get(bot_id .. "Admin:Group:Reply" .. chat_id) or
                     redis:get(bot_id .. "Add:Validity:Users" .. chat_id .. user_id) or "الادمن"
    elseif redis:sismember(bot_id .. 'Vip:Group' .. chat_id, user_id) then
        Status = redis:get(bot_id .. "Vip:Group:Reply" .. chat_id) or
                     redis:get(bot_id .. "Add:Validity:Users" .. chat_id .. user_id) or "المميز"
    else
        Status = redis:get(bot_id .. "Mempar:Group:Reply" .. chat_id) or
                     redis:get(bot_id .. "Add:Validity:Users" .. chat_id .. user_id) or "العضو"
    end
    return Status
end
-- web site : https://planther-ash.com .
...
function send_inline_keyboard(chat_id, text, keyboard, inline, reply_id)
    ...
    local Status_Api = "https://api.telegram.org/bot" .. token .. "/sendMessage?chat_id=" .. chat_id .. "&text=" ..
...
function Send_Options(msg, user_id, status, text)
    tdcli_function({
        ...
        if data.first_name_ ~= false then
            local UserName = (data.username_ or "ufuuu")
...
function Filesplanther(msg)
    ...
    if File_Bot.planther and msg then
        Text_File = File_Bot.planther(msg)
    end
    send(msg.chat_id_, msg.id_, Text_File)
    return false
end
function FilesplantherBot(msg)
    for v in io.popen('ls Files'):lines() do
        if v:match(".lua$") then
            Text_FileBot = dofile("Files/" .. v)
            if Text_FileBot.plantherFile and msg then
                Text_FileBot = Text_FileBot.plantherFile(msg)
            end
        end
    end
...
function SetFile_Groups(msg, chat, File_id, JsonFile)
    ...
    for Id_Group, Info_Group in pairs(JsonInfo.Groups) do
        ...
        list = {"Status:Lock:Bot:kick", "Status:Lock:User:Name", "Status:Lock:hashtak", "Status:Lock:Cmd",
                "Status:Lock:Link", "Status:Lock:forward", "Status:Lock:Keyboard", "Status:Lock:geam",
                "Status:Lock:Photo", "Status:Lock:Animation", "Status:Lock:Video", "Status:Lock:Audio",
                "Status:Lock:vico", "Status:Lock:Sticker", "Status:Lock:Document", "Status:Lock:Unsupported",
                "Status:Lock:Markdaun", "Status:Lock:Contact", "Status:Status:Lock:Spam"}
...
function Dev_planther_File(msg, data)
    if msg then
        ...
        local function porgbot(msg)
            deved = false
            local Status = redis:sismember(bot_id .. "Developer:Bot", msg.sender_user_id_)
            if Status then
                deved = true
            end
            if Dev_planther(msg) == true then
                deved = true
            end
            return deved
        end
        ...
        if msg.content_.text_ then
            ...
            sens = 400
            if redis:get(bot_id .. "Status:Lock:Spam" .. msg.chat_id_) == "del" and not Vips(msg) and
                string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then
                Delete_Message(msg.chat_id_, {
                    [0] = msg.id_
                })
            elseif redis:get(bot_id .. "Status:Lock:Spam" .. msg.chat_id_) == "ked" and not Vips(msg) and
                string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then
                RestrictChat(msg.chat_id_, msg.sender_user_id_)
                Delete_Message(msg.chat_id_, {
                    [0] = msg.id_
                })
            elseif redis:get(bot_id .. "Status:Lock:Spam" .. msg.chat_id_) == "kick" and not Vips(msg) and
                string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then
                KickGroup(msg.chat_id_, msg.sender_user_id_)
                Delete_Message(msg.chat_id_, {
                    [0] = msg.id_
                })
            elseif redis:get(bot_id .. "Status:Lock:Spam" .. msg.chat_id_) == "ktm" and not Vips(msg) and
                string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then
                redis:sadd(bot_id .. "Silence:User:Group" .. msg.chat_id_, msg.sender_user_id_)
                Delete_Message(msg.chat_id_, {
                    [0] = msg.id_
                ...
                tdcli_function({
                    ...
                    if string.len(taha) > tonumber(Num) then
                        ...
                        https.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" ..
                                          msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_)
                    end
                end, nil)
                return false
            end
        ...
        if msg.content_.ID ~= "MessageChatAddMembers" and
            ...
            if msg.sender_user_id_ ~= bot_id and not Vips(msg) then
                ...
                local post_count = tonumber(redis:get(bot_id .. "Spam:Cont" .. msg.sender_user_id_ .. ":" ..
                                                          msg.chat_id_) or 0)
                ...
                redis:setex(bot_id .. "Spam:Cont" .. msg.sender_user_id_ .. ":" .. msg.chat_id_,
                    tonumber(redis:hget(bot_id .. "Spam:Group:User" .. msg.chat_id_, "Num:Spam:Time") or 3),
                    post_count + 1)
            ...
            if redis:get(bot_id .. "Set:Chat:Photo" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
                ...
                tdcli_function({
                    ...
                    if data.code_ == 3 then
                        send(msg.chat_id_, msg.id_,
                            "⌑︙عذرا البوت ليس ادمن يرجى ترقيتي والمحاوله لاحقا")
                        redis:del(bot_id .. "Set:Chat:Photo" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
                        return false
                    end
                    if data.message_ == "CHAT_ADMIN_REQUIRED" then
                        send(msg.chat_id_, msg.id_,
                            "⌑︙ليس لدي صلاحية تغيير معلومات المجموعه يرجى المحاوله لاحقا")
                        redis:del(bot_id .. "Set:Chat:Photo" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
                    else
        ...
        if redis:get(bot_id .. "Broadcasting:Groups:Pin" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
            ...
            send(msg.chat_id_, msg.id_,
                "⌑︙تمت الاذاعه الى *- " .. #list .. " * مجموعه في البوت ")
            if msg.content_.text_ then
                for k, v in pairs(list) do
        ...
        if redis:get(bot_id .. "Broadcasting:Users" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
            ...
            send(msg.chat_id_, msg.id_,
                "⌑︙تمت الاذاعه الى *- " .. #list .. " * مشترك في البوت ")
            if msg.content_.text_ then
                for k, v in pairs(list) do
                    send(v, 0, "[" .. msg.content_.text_ .. "]")
                end
        ...
        if redis:get(bot_id .. "Broadcasting:Groups:Fwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
            ...
            if msg.forward_info_ then
                local list = redis:smembers(bot_id .. "ChekBotAdd")
                send(msg.chat_id_, msg.id_,
                    "⌑︙تم التوجيه الى *- " .. #list .. " * مجموعه في البوت ")
                for k, v in pairs(list) do
                    tdcli_function({
                        ID = "ForwardMessages",
                        chat_id_ = v,
        ...
        if redis:get(bot_id .. "Status:GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then
            if text and text:match("^(%d+)$") then
                ...
                if tonumber(NUM) > 20 then
                    send(msg.chat_id_, msg.id_,
                        "⌑︙عذرآ لا يمكنك تخمين عدد اكبر من ال { 20 } خمن رقم ما بين ال{ 1 و 20 }\n")
                    return false
                end
        ...
        if redis:get(bot_id .. "Status:SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then
            if text and text:match("^(%d+)$") then
                ...
                if tonumber(NUM) > 6 then
                    send(msg.chat_id_, msg.id_,
                        "⌑︙عذرا لا يوجد سواء { 6 } اختيارات فقط ارسل اختيارك مره اخرى\n")
                    return false
                end
                ...
                if tonumber(NUM) == tonumber(GETNUM) then
                    ...
                    send(msg.chat_id_, msg.id_,
                        "⌑︙مبروك فزت وطلعت المحيبس بل ايد رقم { " .. NUM ..
        ...
        if text and redis:get(bot_id .. 'GetTexting:Devplanther' .. msg.chat_id_ .. ':' .. msg.sender_user_id_) then
            ...
            redis:set(bot_id .. 'Texting:Devplanther', text)
            redis:del(bot_id .. 'GetTexting:Devplanther' .. msg.chat_id_ .. ':' .. msg.sender_user_id_)
        ...
        if text or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ or msg.content_.audio_ or
            ...
            if redis:get(bot_id .. "Set:Manager:rd" .. msg.sender_user_id_ .. ":" .. msg.chat_id_) == "true1" then
                ...
                if msg.content_.sticker_ then
                    redis:set(bot_id .. "Add:Rd:Manager:Stekrs" .. test .. msg.chat_id_,
                        msg.content_.sticker_.sticker_.persistent_id_)
                end
                if msg.content_.voice_ then
                    redis:set(bot_id .. "Add:Rd:Manager:Vico" .. test .. msg.chat_id_,
                        msg.content_.voice_.voice_.persistent_id_)
                end
                if msg.content_.animation_ then
                    redis:set(bot_id .. "Add:Rd:Manager:Gif" .. test .. msg.chat_id_,
                ...
                if msg.content_.audio_ then
                    redis:set(bot_id .. "Add:Rd:Manager:Audio" .. test .. msg.chat_id_,
                        msg.content_.audio_.audio_.persistent_id_)
                end
                if msg.content_.document_ then
                    redis:set(bot_id .. "Add:Rd:Manager:File" .. test .. msg.chat_id_,
                        msg.content_.document_.document_.persistent_id_)
                end
                if msg.content_.video_ then
                    redis:set(bot_id .. "Add:Rd:Manager:Video" .. test .. msg.chat_id_,
        ...
        if text and text:match("^(.*)$") then
            if redis:get(bot_id .. "ch:onwer" .. msg.sender_user_id_ .. ":" .. msg.chat_id_) == "true" then
                local Devplanther_Info_Sudo = io.open("info_bot.lua", 'w')
                Devplanther_Info_Sudo:write([[
        do 
        local planther_INFO = {
        id_por = ]] .. text .. [[,
        UserName_planther = "]] .. UserName_Dev .. [[",
        token = "]] .. token .. [["
        }
        return planther_INFO
        end
                ...
                Devplanther_Info_Sudo:close()
                send(msg.chat_id_, msg.id_, '\n⌑︙تم تغير المطور ')
                redis:del(bot_id .. "ch:onwer" .. msg.sender_user_id_ .. ":" .. msg.chat_id_)
                return false
            end
        end
        -- web site : https://planther-ash.com .
            ...
            if not redis:sismember(bot_id .. 'Spam_For_Bot' .. msg.sender_user_id_, text) then
                ...
                if Text then
                    tdcli_function({
                        ...
                        local NumMsgg = redis:get(bot_id .. 'Num:Message:Userr' .. msg.chat_id_ .. ':' ..
                                                      msg.sender_user_id_) or 0
                        local NumMsg = (redis:get(bot_id .. 'Num:Message:User' .. msg.chat_id_ .. ':' ..
                                                      msg.sender_user_id_) or 0 + NumMsgg)
                        local TotalMsg = Total_message(NumMsg)
                        local Status_Gps = Get_Rank(msg.sender_user_id_, msg.chat_id_)
                        local NumMessageEdit = redis:get(bot_id .. 'Num:Message:Edit' .. msg.chat_id_ ..
                                                             msg.sender_user_id_) or 0
        ...
        if Dev_planther(msg) then
            if text == 'نقل الاحصائيات' then
                local Users = redis:smembers(bot_id .. "planther:UsersBot")
                local Groups = redis:smembers(bot_id .. 'planther:Chek:Groups')
                for i = 1, #Groups do
                    redis:sadd(bot_id .. 'ChekBotAdd', Groups[i])
                    local list1 = redis:smembers(bot_id .. 'planther:Basic:Constructor' .. Groups[i])
                    for k, v in pairs(list1) do
                        redis:sadd(bot_id .. "President:User" .. Groups[i], v)
                    end
                    local list2 = redis:smembers(bot_id .. 'planther:Constructor' .. Groups[i])
                    for k, v in pairs(list2) do
                        redis:sadd(bot_id .. "Constructor:Group" .. Groups[i], v)
                    end
                    local list3 = redis:smembers(bot_id .. 'planther:Manager' .. Groups[i])
                    for k, v in pairs(list3) do
                        redis:sadd(bot_id .. "Manager:Group" .. Groups[i], v)
                    end
                    local list4 = redis:smembers(bot_id .. 'planther:Mod:User' .. Groups[i])
                    ...
                    list = {"Status:Lock:Bot:kick", "Status:Lock:User:Name", "Status:Lock:hashtak", "Status:Lock:Cmd",
                            "Status:Lock:Link", "Status:Lock:forward", "Status:Lock:Keyboard", "Status:Lock:geam",
                            "Status:Lock:Photo", "Status:Lock:Animation", "Status:Lock:Video", "Status:Lock:Audio",
                            "Status:Lock:vico", "Status:Lock:Sticker", "Status:Lock:Document",
                            "Status:Lock:Unsupported", "Status:Lock:Markdaun", "Status:Lock:Contact",
                            "Status:Status:Lock:Spam"}
            ...
            elseif text == "متجر الملفات" or text == 'المتجر' then
                local Get_Files, res = https.request(
                                           "https://raw.githubusercontent.com/plantherTeam/Files_planther/master/getfile.json")
                if res == 200 then
                    local Get_info, res = pcall(JSON.decode, Get_Files);
                    if Get_info then
                        local TextS =
                            "\n⌑︙قائمه ملفات متجر سورس planther\n⌑︙الملفات المتوفره حاليا\n━━━━━━━━━━━━━\n\n"
                        local TextE =
            ...
            elseif text and text:match("^(تعطيل ملف) (.*)(.lua)$") then
                ...
                local Get_Json, Res = https.request(
                                          "https://raw.githubusercontent.com/plantherTeam/Files_planther/master/Files_planther/" ..
                                              File_Name)
                if Res == 200 then
        ...
        if TypeForChat == ("ForUser") then
            if text == '/start' then
                ...
                if Dev_planther(msg) then
                    ...
                    local List_keyboard = {{'تفعيل تواصل البوت 🔔',
                                            'تعطيل تواصل البوت 🔕'},
                                           ...
                                           {'تفعيل مغادرة البوت ✔',
                                            'تعطيل مغادرة البوت ❌'},
                                           {'تفعيل اذاعه المطورين 🔓',
                                            'تعطيل اذاعه المطورين 🔏'},
                                           {'تفعيل الوضع الخدمي 🔖',
                                            'تعطيل الوضع الخدمي 〽'},
            ...
            if not Dev_planther(msg) and not redis:sismember(bot_id .. 'User:Ban:Pv', msg.sender_user_id_) and
                not redis:get(bot_id .. 'Status:Lock:Twasl') then
                send(msg.sender_user_id_, msg.id_,
                    '⌑︙تم ارسال رسالتك الى المطور ← { [' .. UserName_Dev .. '] }')
            ...
            if Dev_planther(msg) then
                if msg.reply_to_message_id_ ~= 0 then
                    tdcli_function({
                        ...
                        tdcli_function({
                        ...
                        }, function(arg, Get_Status)
                            if (Get_Status.code_) == (400) or (Get_Status.code_) == (5) then
                                Send_Optionspv(Id_Dev, msg.id_, UserForward, "reply_Pv",
                                    "⌑︙قام بحظر البوت لا تستطيع ارسال له رسائل")
                                return false
                            end
                            if text then
                            ...
                            elseif msg.content_.ID == 'MessagePhoto' then
                                sendPhoto(UserForward, msg.id_, msg.content_.photo_.sizes_[0].photo_.persistent_id_,
                ...
                elseif text == "اذاعه للمجموعات 👥" then
                    ...
                    send(msg.chat_id_, msg.id_,
                        "⌑︙ارسل لي المنشور الان\n⌑︙يمكنك ارسال -{ صوره - ملصق - متحركه - رساله }\n⌑︙لالغاء الاذاعه ارسل : الغاء")
                    return false
                elseif text == "اذاعه بالتثبيت 📌" and porgbot(msg) then
                    redis:setex(bot_id .. "Broadcasting:Groups:Pin" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600,
                        true)
                    send(msg.chat_id_, msg.id_,
                        "⌑︙ارسل لي المنشور الان\n⌑︙يمكنك ارسال -{ صوره - ملصق - متحركه - رساله }\n⌑︙لالغاء الاذاعه ارسل : الغاء")
                    return false
                elseif text == "اذاعه بالتوجيه 👥" and porgbot(msg) then
                    redis:setex(bot_id .. "Broadcasting:Groups:Fwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600,
                        true)
                    send(msg.chat_id_, msg.id_,
                        "⌑︙ارسل لي التوجيه الان\n⌑︙ليتم نشره في المجموعات")
                    return false
                elseif text == "اذاعه خاص بالتوجيه 👤" and porgbot(msg) then
                    redis:setex(bot_id .. "Broadcasting:Users:Fwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600,
                        true)
                    send(msg.chat_id_, msg.id_,
                        "⌑︙ارسل لي التوجيه الان\n⌑︙ليتم نشره الى المشتركين")
                    return false
                elseif text == 'ازالة كليشة المطور 🆗' then
                    redis:del(bot_id .. 'Texting:Devplanther')
                ...
                elseif text and text:match("^حظر عام @(.*)$") then
                    function FunctionStatus(arg, result)
                        if (result.id_) then
                            ...
                            if por_bot(result.id_) == true then
                                send(msg.chat_id_, msg.id_,
                                    "⌑︙لا تستطيع حظر المطور الاساسي عام")
                                return false
                            end
                ...
                elseif text and text:match("^الغاء العام @(.*)$") then
                    ...
                    tdcli_function({
                        ID = "SearchPublicChat",
                        username_ = text:match("^الغاء العام @(.*)$")
                    }, FunctionStatus, nil)
                ...
                elseif text and text:match("^حذف مطور @(.*)$") then
                    ...
                    tdcli_function({
                        ID = "SearchPublicChat",
                        username_ = text:match("^حذف مطور @(.*)$")
                    }, FunctionStatus, nil)
                elseif text == 'احصائيات البوت 📑' then
                    send(msg.chat_id_, msg.id_,
                        '*⌑︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌑︙عدد المجموعات : ' ..
                            (redis:scard(bot_id .. 'ChekBotAdd') or 0) .. '\n⌑︙عدد المشتركين : ' ..
                            (redis:scard(bot_id .. 'Num:User:Pv') or 0) .. '*')
                elseif text and text:match("^تعين عدد الاعضاء (%d+)$") then
                    redis:set(bot_id .. 'Num:Add:Bot', text:match("تعين عدد الاعضاء (%d+)$"))
                    send(msg.chat_id_, msg.id_,
                        '*⌑︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : ' ..
                            text:match("تعين عدد الاعضاء (%d+)$") .. ' عضو *')
                ...
                elseif text == 'جلب نسخة خزن الكروبات 📦' then
                    ...
                    Get_Json = Get_Json .. '}}'
                    local File = io.open('./Files/' .. bot_id .. '.json', "w")
                    File:write(Get_Json)
                    File:close()
                    sendDocument(msg.chat_id_, msg.id_, './Files/' .. bot_id .. '.json',
        ...
        if TypeForChat == ("ForSuppur") then
            if text == "" .. Name_Bot .. ' شنو رئيك بهاذا' and tonumber(msg.reply_to_message_id_) > 0 and
                not redis:get(bot_id .. "Status:Fun:Bots" .. msg.chat_id_) then
                tdcli_function({
                    ID = "GetMessage",
                    chat_id_ = msg.chat_id_,
                    message_id_ = tonumber(msg.reply_to_message_id_)
                }, function(Arg, Data)
                    local Text_Fun = {'لوكي وزاحف من ساع زحفلي وحضرته 😒',
                                      ...
                                      'مو خوش ولد صراحه ☹️',
                                      'ادبسز وميحترم البنات  ', 'فد واحد قذر 🙄😒',
                                      'ماطيقه كل ما اكمشه ريحته جنها بخاخ بف باف مال حشرات 😂🤷‍♀️',
                                      'مو خوش ولد 🤓'}
                    send(msg.chat_id_, Data.id_, '' .. Text_Fun[math.random(#Text_Fun)] .. '')
                end, nil)
                return false
            elseif text == "" .. Name_Bot .. ' شنو رئيك بهاي' and tonumber(msg.reply_to_message_id_) > 0 and
                not redis:get(bot_id .. "Status:Fun:Bots" .. msg.chat_id_) then
                tdcli_function({
                    ID = "GetMessage",
                    chat_id_ = msg.chat_id_,
                    ...
                    local Text_Fun = {'الكبد مال اني هيه ', 'ختولي ماحبها ',
                                      'خانتني ويه صديقي 😔',
            ...
            if text then
                local NewCmmd = redis:get(bot_id .. "Set:Za:Group:New1" .. msg.chat_id_ .. ':' ..
                                              data.message_.content_.text_)
                if NewCmmd then
                    data.message_.content_.text_ = (NewCmmd or data.message_.content_.text_)
                end
            end
            if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and
                ...
                tdcli_function({
                    ...
                    if Data.content_.document_ then
                        SetFile_Groups(msg, msg.chat_id_, Data.content_.document_.document_.persistent_id_,
                            Data.content_.document_.file_name_)
                    end
                end, nil)
            end
            ...
            if text and text:match("^تحميل (.*)$") then
                ...
                if (Json_Info.url) then
                    ...
                    send(msg.chat_id_, msg.id_, "⌑︙حدث خطأ لابمكن التحميل ")
                end
            end
            ...
            if text == 'جلب نسخه احتياطيه' and Dev_planther(msg) or text ==
                'جلب نسخه الكروبات' and Dev_planther(msg) then
                local list = redis:smembers(bot_id .. 'ChekBotAdd')
            ...
            if text == 'تنزيل جميع الرتب' and PresidentGroup(msg) then
                ...
                send(msg.chat_id_, msg.id_,
                    '⌑︙تم تنزيل الكل من جميع الرتب\n{الاساسين، المنشئين ، المدراء ، الادمنيه ، المميزين}')
            end

            if text == ("مسح قائمه العام") and Dev_planther(msg) or text ==
            ...
            elseif text == ("مسح المطورين") and Dev_planther(msg) then
                ...
                redis:del(bot_id .. "Developer:Bot")
                send(msg.chat_id_, msg.id_, "⌑︙ تم مسح المطورين من البوت  ")
            elseif text == ("مسح المنشئين الاساسين") and PresidentGroup(msg) or text ==
                "مسح الاساسين" and porgbot(msg) then
                if AddChannel(msg.sender_user_id_) == false then
                    send(msg.chat_id_, msg.id_,
                        '\n⌑︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌑︙قناة البوت 📁.\n- @ufuuu')
                    return false
                ...
                redis:del(bot_id .. "Basic:User" .. msg.chat_id_)
                send(msg.chat_id_, msg.id_,
                    "⌑︙ تم مسح المنشئين الاساسيين في المجموعه")
            ...
            elseif text == "حذف الاوامر المضافه" and Constructor(msg) or text ==
            ...
            elseif text == ("قائمه العام") and Dev_planther(msg) or text == ("المحظورين عام") and
                Dev_planther(msg) then
                if AddChannel(msg.sender_user_id_) == false then
                    send(msg.chat_id_, msg.id_,
                        '\n⌑︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌑︙قناة البوت 📁.\n- @ufuuu')
            ...
            elseif text == ("المطورين") and Dev_planther(msg) then
                ...
                send(msg.chat_id_, msg.id_, Sudos)
            elseif text == "المنشئين الاساسين" and PresidentGroup(msg) or text == "الاساسين" and
                porgbot(msg) then
                if AddChannel(msg.sender_user_id_) == false then
                    send(msg.chat_id_, msg.id_,
                        '\n⌑︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌑︙قناة البوت 📁.\n- @ufuuu')
            ...
            elseif text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_planther(msg) then
                ...
                function FunctionStatus(arg, result)
                    ...
                    if por_bot(result.sender_user_id_) == true then
            ...
            elseif text == ("رفع منشئ اساسي") and tonumber(msg.reply_to_message_id_) ~= 0 and
            ...
            elseif text == ("تنزيل منشئ اساسي") and tonumber(msg.reply_to_message_id_) ~= 0 and
                PresidentGroup(msg) then
                if AddChannel(msg.sender_user_id_) == false then
                    send(msg.chat_id_, msg.id_,
                        '\n⌑︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌑︙قناة البوت 📁.\n- @ufuuu')
            ...
            elseif text and text:match("^تنزيل منشئ$") and tonumber(msg.reply_to_message_id_) ~= 0 and
            ...
            elseif text == ("رفع ادمن") and tonumber(msg.reply_to_message_id_) ~= 0 and Owner(msg) then
                ...
                if not Constructor(msg) and redis:get(bot_id .. "Status:Cheking:Seted" .. msg.chat_id_) then
                    send(msg.chat_id_, msg.id_,
                        '⌑︙لا تستطيع رفع احد وذالك لان تم تعطيل الرفع من قبل المنشئين')
                    return false
                end
                function FunctionStatus(arg, result)
            ...
            elseif text == ("حظر") and msg.reply_to_message_id_ ~= 0 and Admin(msg) then
                ...
                if not Constructor(msg) and redis:get(bot_id .. "Status:Lock:Ban:Group" .. msg.chat_id_) then
                    send(msg.chat_id_, msg.id_,
                        '⌑︙لقد تم تعطيل الحظر و الطرد من قبل المنشئين')
                    return false
                end
                    ...
                    if Rank_Checking(result.sender_user_id_, msg.chat_id_) == true then
                        ...
                        tdcli_function({
                            ...
                            if (data and data.code_ and data.code_ == 400 and data.message_ == "CHAT_ADMIN_REQUIRED") then
                                send(msg.chat_id_, msg.id_,
                                    "⌑︙لا توجد لدي صلاحية حظر المستخدمين")
                                return false
                            end
            ...
            elseif text and text:match("^رفع منشئ اساسي @(.*)$") and PresidentGroup(msg) then
                ...
                tdcli_function({
                    ID = "SearchPublicChat",
                    username_ = text:match("^رفع منشئ اساسي @(.*)$")
                }, FunctionStatus, nil)
            elseif text and text:match("^تنزيل منشئ اساسي @(.*)$") and PresidentGroup(msg) then
                ...
                tdcli_function({
                    ID = "SearchPublicChat",
                    username_ = text:match("^تنزيل منشئ اساسي @(.*)$")
                }, FunctionStatus, nil)
            ...
            elseif text and text:match("^تنزيل منشئ @(.*)$") and BasicBuilder(msg) then
                ...
                tdcli_function({
                    ID = "SearchPublicChat",
                    username_ = text:match("^تنزيل منشئ @(.*)$")
                }, FunctionStatus, nil)
            ...
            elseif text and text:match("^تنزيل مدير @(.*)$") and Constructor(msg) then
                ...
                tdcli_function({
                    ID = "SearchPublicChat",
                    username_ = text:match("^تنزيل مدير @(.*)$")
                }, FunctionStatus, nil)
            ...
            elseif text and text:match("^تنزيل ادمن @(.*)$") and Owner(msg) then
                ...
                tdcli_function({
                    ID = "SearchPublicChat",
                    username_ = text:match("^تنزيل ادمن @(.*)$")
                }, FunctionStatus, nil)
            ...
            elseif text and text:match("^تنزيل مميز @(.*)$") and Admin(msg) then
                ...
                tdcli_function({
                    ID = "SearchPublicChat",
                    username_ = text:match("^تنزيل مميز @(.*)$")
                }, FunctionStatus, nil)
            elseif text and text:match("رفع (.*)") and tonumber(msg.reply_to_message_id_) > 0 and Admin(msg) then
                if redis:sismember(bot_id .. "Validitys:Group" .. msg.chat_id_, text:match("رفع (.*)")) then
                    function Status_reply(extra, result, success)
                        local statusrt = redis:get(bot_id .. "Add:Validity:Group:Rt" .. text:match("رفع (.*)") ..
                                                       msg.chat_id_)
                        if statusrt == "مميز" and Admin(msg) then
                            redis:set(bot_id .. "Add:Validity:Users" .. msg.chat_id_ .. result.sender_user_id_,
                                text:match("رفع (.*)"))
                            redis:sadd(bot_id .. "Vip:Group" .. msg.chat_id_, result.sender_user_id_)
            ...
            elseif text and text:match("تنزيل (.*)") and tonumber(msg.reply_to_message_id_) > 0 and Admin(msg) then
                if redis:sismember(bot_id .. "Validitys:Group" .. msg.chat_id_, text:match("تنزيل (.*)")) then
                    function Status_reply(extra, result, success)
                        local statusrt = redis:get(bot_id .. "Add:Validity:Group:Rt" .. text:match("تنزيل (.*)") ..
                                                       msg.chat_id_)
                        if statusrt == "مميز" and Admin(msg) then
                            redis:srem(bot_id .. "Vip:Group" .. msg.chat_id_, result.sender_user_id_)
            ...
            elseif text and text:match("^الغاء حظر @(.*)$") and Admin(msg) then
                ...
                function FunctionStatus(arg, result)
                    if (result.id_) then
                        if tonumber(result.id_) == tonumber(bot_id) then
                            send(msg.chat_id_, msg.id_,
                                "⌑︙لا يمكنك عمل هاذا الامر على البوت")
                            return false
                        end
                ...
                tdcli_function({
                    ID = "SearchPublicChat",
                    username_ = text:match("^الغاء حظر @(.*)$")
                }, FunctionStatus, nil)
            ...
            elseif text and text:match("^الغاء كتم @(.*)$") and Admin(msg) then
                ...
                tdcli_function({
                    ID = "SearchPublicChat",
                    username_ = text:match("^الغاء كتم @(.*)$")
                }, FunctionStatus, nil)
                ...
                function FunctionStatus(arg, result)
                    if (result.id_) then
                        ...
                        if Rank_Checking(result.id_, msg.chat_id_) then
                            ...
                            https.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" ..
                                              msg.chat_id_ .. "&user_id=" .. result.id_ .. '&until_date=' ..
                                              tonumber(msg.date_ + Time))
                        end
                    end
                end
                tdcli_function({
            ...
            elseif text and text:match("^الغاء تقيد @(.*)$") and Admin(msg) then
                ...
                tdcli_function({
                    ID = "SearchPublicChat",
                    username_ = text:match("^الغاء تقيد @(.*)$")
                }, FunctionStatus, nil)
            ...
            elseif text and text:match("^حظر عام (%d+)$") and Dev_planther(msg) then
                ...
                if por_bot(text:match("^حظر عام (%d+)$")) == true then
                    send(msg.chat_id_, msg.id_, "⌑︙لا تستطيع حظر المطور الاساسي عام")
                    return false
                end
            ...
            elseif text and text:match("^حظر (%d+)$") and Admin(msg) then
                ...
                if Rank_Checking(text:match("^حظر (%d+)$"), msg.chat_id_) == true then
                    send(msg.chat_id_, msg.id_,
                        "\n⌑︙لا تستطيع -( حظر , طرد , كتم , تقيد ) : " ..
                            Get_Rank(text:match("^حظر (%d+)$"), msg.chat_id_) .. "")
                else
                    tdcli_function({
                        ID = "ChangeChatMemberStatus",
                        chat_id_ = msg.chat_id_,
                        user_id_ = text:match("^حظر (%d+)$"),
                        status_ = {
                            ID = "ChatMemberStatusKicked"
                        }
                    }, function(arg, data)
            ...
            elseif text and text:match("^الغاء حظر (%d+)$") and Admin(msg) then
                ...
                tdcli_function({
                    ID = "ChangeChatMemberStatus",
                    chat_id_ = msg.chat_id_,
                    user_id_ = text:match("^الغاء حظر (%d+)$"),
                    status_ = {
                        ID = "ChatMemberStatusLeft"
                    }
                }, function(arg, ban)
            ...
            elseif text and text:match("^كتم (%d+)$") and Admin(msg) then
                ...
                if Rank_Checking(text:match("^كتم (%d+)$"), msg.chat_id_) == true then
                    send(msg.chat_id_, msg.id_,
                        "\n⌑︙لا تستطيع -( حظر , طرد , كتم , تقيد ) : " ..
                            Get_Rank(text:match("^كتم (%d+)$"), msg.chat_id_) .. "")
                else
                    if msg.can_be_deleted_ == false then
                        send(msg.chat_id_, msg.id_, "⌑︙عذرآ البوت ليس ادمن")
                        return false
            ...
            elseif text and text:match("^تقيد (%d+)$") and Admin(msg) then
                ...
                if Rank_Checking(text:match("^تقيد (%d+)$"), msg.chat_id_) then
                    send(msg.chat_id_, msg.id_,
                        "\n⌑︙لا تستطيع -( حظر , طرد , كتم , تقيد ) : " ..
                            Get_Rank(text:match("^تقيد (%d+)$"), msg.chat_id_) .. "")
                else
                    https.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" ..
                                      msg.chat_id_ .. "&user_id=" .. text:match("^تقيد (%d+)$"))
                    Send_Options(msg, text:match("^تقيد (%d+)$"), "reply",
                        "⌑︙تم تقييده في المجموعه")
                end
            elseif text and text:match('^تقيد (%d+) (.*)$') and tonumber(msg.reply_to_message_id_) ~= 0 and
                Admin(msg) then
                local TextEnd = {string.match(text, "^(تقيد) (%d+) (.*)$")}
                if msg.can_be_deleted_ == false then
                    send(msg.chat_id_, msg.id_, "⌑︙عذرآ البوت ليس ادمن")
                ...
                function FunctionStatus(arg, result)
                    ...
                    if Rank_Checking(result.sender_user_id_, msg.chat_id_) then
                        ...
                        Send_Options(msg, result.sender_user_id_, "reply",
                            "⌑︙تم تقيده لمدة ~ { " .. TextEnd[2] .. ' ' .. TextEnd[3] .. '}')
                        https.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" ..
                                          msg.chat_id_ .. "&user_id=" .. result.sender_user_id_ .. '&until_date=' ..
                                          tonumber(msg.date_ + Time))
                    end
                end
                tdcli_function({
            ...
            elseif text and text:match("^الغاء تقيد (%d+)$") and Admin(msg) then
                ...
                https.request(
                    "https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ ..
                        "&user_id=" .. text:match("^الغاء تقيد (%d+)$") ..
                        "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
            ...
            elseif text and text:match("^طرد (%d+)$") and Admin(msg) then
                ...
                if Rank_Checking(text:match("^طرد (%d+)$"), msg.chat_id_) == true then
                    send(msg.chat_id_, msg.id_,
                        "\n⌑︙لا تستطيع -( حظر , طرد , كتم , تقيد ) : " ..
                            Get_Rank(text:match("^طرد (%d+)$"), msg.chat_id_) .. "")
                else
                    tdcli_function({
                        ID = "ChangeChatMemberStatus",
                        chat_id_ = msg.chat_id_,
                        user_id_ = text:match("^طرد (%d+)$"),
                        status_ = {
                            ID = "ChatMemberStatusKicked"
                        }
                    }, function(arg, data)
            ...
            elseif text == "قفل التثبيت" and msg.reply_to_message_id_ == 0 and Constructor(msg) then
                ...
                tdcli_function({
                    ...
                    redis:set(bot_id .. "Get:Id:Msg:Pin" .. msg.chat_id_, data.pinned_message_id_)
                end, nil)
            ...
            elseif text == "تفعيل الحمايه القصوى" and Owner(msg) then
                ...
                redis:set(bot_id .. "Status:Lock:tagservrbot" .. msg.chat_id_, true)
                list = {"Status:lock:Far", "Status:lock:Fsh", "Status:Lock:Bot:kick", "Status:Lock:User:Name",
                        "Status:Lock:hashtak", "Status:Lock:Cmd", "Status:Lock:Link", "Status:Lock:forward",
                        "Status:Lock:Keyboard", "Status:Lock:geam", "Status:Lock:Photo", "Status:Lock:Animation",
                        "Status:Lock:Video", "Status:Lock:Audio", "Status:Lock:vico", "Status:Lock:Sticker",
                        "Status:Lock:Document", "Status:Lock:Unsupported", "Status:Lock:Markdaun",
                        "Status:Lock:Contact", "Status:Status:Lock:Spam"}
                for i, lock in pairs(list) do
            ...
            elseif text == "تفعيل جلب الرابط" and Admin(msg) or text == 'تفعيل الرابط' and
                Admin(msg) then
                redis:del(bot_id .. "Link_Group" .. msg.chat_id_)
                send(msg.chat_id_, msg.id_, "⌑︙تم تفعيل جلب الرابط المجموعه")
            elseif text == "تعطيل جلب الرابط" and Admin(msg) or text == 'تعطيل الرابط' and
                Admin(msg) then
                redis:set(bot_id .. "Link_Group" .. msg.chat_id_, true)
            ...
            elseif text == "تفعيل المغادره" and Dev_planther(msg) then
                redis:del(bot_id .. "Status:Lock:Left" .. msg.chat_id_)
                send(msg.chat_id_, msg.id_, "⌑︙تم تفعيل مغادرة البوت")
            elseif text == "تعطيل المغادره" and Dev_planther(msg) then
                redis:set(bot_id .. "Status:Lock:Left" .. msg.chat_id_, true)
                send(msg.chat_id_, msg.id_, "⌑︙تم تعطيل مغادرة البوت")
            elseif text == "تفعيل الاذاعه" and Dev_planther(msg) then
                redis:del(bot_id .. "Status:Broadcasting:Bot")
                send(msg.chat_id_, msg.id_,
                    "⌑︙تم تفعيل الاذاعه \n⌑︙الان يمكن للمطورين الاذاعه")
            elseif text == "تعطيل الاذاعه" and Dev_planther(msg) then
                redis:set(bot_id .. "Status:Broadcasting:Bot", true)
                send(msg.chat_id_, msg.id_, "⌑︙تم تعطيل الاذاعه")
            ...
            elseif text == 'تفعيل البوت الخدمي' and Dev_planther(msg) then
                redis:del(bot_id .. 'Free:Bot')
                send(msg.chat_id_, msg.id_,
                    '⌑︙تم تفعيل البوت الخدمي \n⌑︙الان يمكن الجميع تفعيله')
            elseif text == 'تعطيل البوت الخدمي' and Dev_planther(msg) then
                redis:set(bot_id .. 'Free:Bot', true)
                send(msg.chat_id_, msg.id_, '⌑︙تم تعطيل البوت الخدمي')
            elseif text == "تعطيل الطرد" and Constructor(msg) or text == "تعطيل الحظر" and
                Constructor(msg) then
                redis:set(bot_id .. "Status:Lock:Ban:Group" .. msg.chat_id_, "true")
                send(msg.chat_id_, msg.id_, '⌑︙تم تعطيل - ( الحظر - الطرد ) ')
            elseif text == "تفعيل الطرد" and Constructor(msg) or text == "تفعيل الحظر" and
                Constructor(msg) then
                redis:del(bot_id .. "Status:Lock:Ban:Group" .. msg.chat_id_)
                send(msg.chat_id_, msg.id_, '⌑︙تم تفعيل - ( الحظر - الطرد ) ')
            elseif text == "تعطيل الرفع" and Constructor(msg) or text == "تعطيل الترقيه" and
                Constructor(msg) then
                redis:set(bot_id .. "Status:Cheking:Seted" .. msg.chat_id_, "true")
                send(msg.chat_id_, msg.id_, '⌑︙تم تعطيل رفع - ( الادمن - المميز ) ')
            elseif text == "تفعيل الرفع" and Constructor(msg) or text == "تفعيل الترقيه" and
                Constructor(msg) then
            ...
            elseif text and text:match("^وضع تكرار (%d+)$") and Admin(msg) then
                redis:hset(bot_id .. "Spam:Group:User" .. msg.chat_id_, "Num:Spam",
                    text:match("^وضع تكرار (%d+)$"))
                send(msg.chat_id_, msg.id_,
                    "⌑︙تم وضع عدد التكرار : " .. text:match("^وضع تكرار (%d+)$") .. "")
            elseif text and text:match("^وضع زمن التكرار (%d+)$") and Admin(msg) then
                redis:hset(bot_id .. "Spam:Group:User" .. msg.chat_id_, "Num:Spam:Time",
                    text:match("^وضع زمن التكرار (%d+)$"))
                send(msg.chat_id_, msg.id_, "⌑︙تم وضع زمن التكرار : " ..
                    text:match("^وضع زمن التكرار (%d+)$") .. "")
            ...
            elseif text and text:match("^ضع صوره") and Admin(msg) and msg.reply_to_message_id_ == 0 or text and
                text:match("^وضع صوره") and Admin(msg) and msg.reply_to_message_id_ == 0 then
            ...
            elseif text == 'وضع كليشه المطور' and Dev_planther(msg) then
                redis:set(bot_id .. 'GetTexting:Devplanther' .. msg.chat_id_ .. ':' .. msg.sender_user_id_, true)
                send(msg.chat_id_, msg.id_, '⌑︙ ارسل لي الكليشه الان')
            elseif text and text:match("^ضع اسم (.*)") and Owner(msg) or text and text:match("^وضع اسم (.*)") and
                Owner(msg) then
                ...
                tdcli_function({
                    ...
                    if data.message_ == "CHAT_ADMIN_REQUIRED" then
                        send(msg.chat_id_, msg.id_,
                            "⌑︙ ليست لدي صلاحية تغير اسم المجموعه")
                    else
                        send(msg.chat_id_, msg.id_,
                            "⌑︙ تم تغيير اسم المجموعه الى {[" .. Name .. "]}")
                    end
                end, nil)
            elseif text == 'روابط الكروبات' and Dev_planther(msg) then
                local list = redis:smembers(bot_id .. 'ChekBotAdd')
                test = '⌑︙روابط الكروبات \n\n'
                for k, v in pairs(list) do
                    ...
                    local linkgpp = json:decode(https.request(
                                                    'https://api.telegram.org/bot' .. token ..
                                                        '/exportChatInviteLink?chat_id=' .. v))
                    if link then
                        test = test .. "" .. k .. "- » " .. v .. "\n { " .. link .. "}\n"
                    else
            ...
            elseif text == "حذف امر" and Constructor(msg) or text == "مسح امر" and Constructor(msg) then
                ...
                send(msg.chat_id_, msg.id_,
                    "⌑︙ارسل الان الامر الذي قمت بوضعه مكان الامر القديم")
            elseif text and text:match("^مسح صلاحيه (.*)$") and Admin(msg) or text and
                text:match("^حذف صلاحيه (.*)$") and Admin(msg) then
            ...
            elseif text == ("ردود المطور") and Dev_planther(msg) then
                local list = redis:smembers(bot_id .. "List:Rd:Sudo")
                text = "\n⌑︙قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
                for k, v in pairs(list) do
                ...
                send(msg.chat_id_, msg.id_, "[" .. text .. "]")
            elseif text == "اضف رد للكل" and Dev_planther(msg) then
                send(msg.chat_id_, msg.id_,
                    "⌑︙ارسل الان الكلمه لاضافتها في ردود المكور ")
                redis:set(bot_id .. "Set:Rd" .. msg.sender_user_id_ .. ":" .. msg.chat_id_, true)
            elseif text == "حذف رد للكل" and Dev_planther(msg) then
                send(msg.chat_id_, msg.id_,
                    "⌑︙ارسل الان الكلمه لحذفها من ردود المطور")
            ...
            if text and text:match("^تنزيل الكل @(.*)$") and Owner(msg) then
                ...
                function FunctionStatus(extra, result, success)
                    if (result.id_) then
                        ...
                        if por_bot(msg.sender_user_id_) == true then
                            redis:srem(bot_id .. "Developer:Bot", result.id_)
                ...
                tdcli_function({
                    ID = "SearchPublicChat",
                    username_ = text:match("^تنزيل الكل @(.*)$")
                }, FunctionStatus, nil)
            end
            ...
            elseif text == "غادر" then
                if porgbot(msg) and not redis:get(bot_id .. "Status:Lock:Left" .. msg.chat_id_) then
                    tdcli_function({
                        ID = "ChangeChatMemberStatus",
                        chat_id_ = msg.chat_id_,
                        user_id_ = bot_id,
            ...
            elseif text == Name_Bot then
                local namebot = {"لابسك لتلح", "لاتوصخ اسمي", "هاقلبي؟ كول",
                                 "دعبل يمعود", "اهو شتريد ؟"}
                name = math.random(#namebot)
                send(msg.chat_id_, msg.id_, namebot[name])
            elseif text == "بوت" then
                local BotName = {"باوع لك خليني احبك وصيحلي باسمي " .. Name_Bot .. "",
                                 "لتخليني ارجع لحركاتي لقديمه وردا ترا اسمي " ..
                    Name_Bot .. "", "راح نموت بكورونا ونته بعدك تصيح بوت"}
                BotNameText = math.random(#BotName)
                send(msg.chat_id_, msg.id_, BotName[BotNameText])
            elseif text == "تغير اسم البوت" and Dev_planther(msg) or text == "تغيير اسم البوت" and
                Dev_planther(msg) then
                redis:setex(bot_id .. "Change:Name:Bot" .. msg.sender_user_id_, 300, true)
                send(msg.chat_id_, msg.id_, "⌑︙ ارسل لي الاسم الان ")
            elseif text == "اذاعه خاص" and msg.reply_to_message_id_ == 0 and porgbot(msg) then
                if redis:get(bot_id .. "Status:Broadcasting:Bot") and not Dev_planther(msg) then
                    send(msg.chat_id_, msg.id_,
                        "⌑︙تم تعطيل الاذاعه من قبل المطور الاساسي !")
                    return false
                end
                redis:setex(bot_id .. "Broadcasting:Users" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true)
                send(msg.chat_id_, msg.id_,
                    "⌑︙ارسل لي المنشور الان\n⌑︙يمكنك ارسال -{ صوره - ملصق - متحركه - رساله }\n⌑︙لالغاء الاذاعه ارسل : الغاء")
                return false
            elseif text == "اذاعه" and msg.reply_to_message_id_ == 0 and porgbot(msg) then
                if redis:get(bot_id .. "Status:Broadcasting:Bot") and not Dev_planther(msg) then
                    send(msg.chat_id_, msg.id_,
                        "⌑︙تم تعطيل الاذاعه من قبل المطور الاساسي !")
                    return false
                ...
                send(msg.chat_id_, msg.id_,
                    "⌑︙ارسل لي المنشور الان\n⌑︙يمكنك ارسال -{ صوره - ملصق - متحركه - رساله }\n⌑︙لالغاء الاذاعه ارسل : الغاء")
                return false
            elseif text == "اذاعه بالتثبيت" and msg.reply_to_message_id_ == 0 and porgbot(msg) then
                if redis:get(bot_id .. "Status:Broadcasting:Bot") and not Dev_planther(msg) then
                    send(msg.chat_id_, msg.id_,
                        "⌑︙تم تعطيل الاذاعه من قبل المطور الاساسي !")
                    return false
                ...
                send(msg.chat_id_, msg.id_,
                    "⌑︙ارسل لي المنشور الان\n⌑︙يمكنك ارسال -{ صوره - ملصق - متحركه - رساله }\n⌑︙لالغاء الاذاعه ارسل : الغاء")
                return false
            elseif text == "اذاعه بالتوجيه" and msg.reply_to_message_id_ == 0 and porgbot(msg) then
                if redis:get(bot_id .. "Status:Broadcasting:Bot") and not Dev_planther(msg) then
                    send(msg.chat_id_, msg.id_,
                        "⌑︙تم تعطيل الاذاعه من قبل المطور الاساسي !")
                    return false
                ...
                send(msg.chat_id_, msg.id_,
                    "⌑︙ارسل لي التوجيه الان\n⌑︙ليتم نشره في المجموعات")
                return false
            elseif text == "اذاعه بالتوجيه خاص" and msg.reply_to_message_id_ == 0 and porgbot(msg) then
                if redis:get(bot_id .. "Status:Broadcasting:Bot") and not Dev_planther(msg) then
                    send(msg.chat_id_, msg.id_,
                        "⌑︙تم تعطيل الاذاعه من قبل المطور الاساسي !")
                    return false
...
.𖣂 𝙢𝙨𝙂𝙨 , #msgs
            ...
            if text == 'كشف' and tonumber(msg.reply_to_message_id_) > 0 and
                not redis:get(bot_id .. 'Status:Lock:Id:Photo' .. msg.chat_id_) then
                function Function_Status(extra, result, success)
                    tdcli_function({
                        ID = "GetUser",
                        user_id_ = result.sender_user_id_
                ...
                return false
            elseif text and text:match("^كشف @(.*)$") and
                not redis:get(bot_id .. 'Status:Lock:Id:Photo' .. msg.chat_id_) then
            ...
            if text == 'ايدي' and tonumber(msg.reply_to_message_id_) == 0 and
                ...
                tdcli_function({
                    ...
                    tdcli_function({
                        ...
                        local NumMsgg1 = redis:get(bot_id .. 'Num:Message:Userr' .. msg.chat_id_ .. ':' ..
                                                       msg.sender_user_id_) or 0
                        local nummsg = redis:get(bot_id .. 'Num:Message:User' .. msg.chat_id_ .. ':' ..
                        ...
                        local NumMessageEdit = redis:get(bot_id .. 'Num:Message:Edit' .. msg.chat_id_ ..
                                                             msg.sender_user_id_) or 0
                        local Num_Games = redis:get(bot_id .. "Num:Add:Games" .. msg.chat_id_ .. msg.sender_user_id_) or
                                              0
                        local Add_Mem =
                            redis:get(bot_id .. "Num:Add:Memp" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) or 0
                        local Total_Photp = (taha.total_count_ or 0)
                        local Texting = {'ملاك وناسيك بكروبنه😟', "حلغوم والله☹️ ",
                                         "اطلق صوره🐼❤️", "كيكك والله🥺",
                                         "لازك بيها غيرها عاد😒"}
            ...
            if text and text:match('^تنظيف (%d+)$') and Admin(msg) or text and text:match('^حذف (%d+)$') and
                Admin(msg) or text and text:match('^مسح (%d+)$') and Admin(msg) then
                local Msg_Num = tonumber(text:match('^تنظيف (%d+)$')) or tonumber(text:match('^حذف (%d+)$')) or
                                    tonumber(text:match('^مسح (%d+)$'))
                ...
                send(msg.chat_id_, msg.id_,
                    '⌑︙تم ازالة *- ' .. Msg_Num .. '* رساله من المجموعه')
            elseif text and (text == "تنظيف جميع الميديا" or text == "تنظيف الميديا") and
                Admin(msg) then
                lk_lkid = {
                    [0] = msg.id_
                }
                ...
                tdcli_function({
                    ID = "GetMessages",
                    chat_id_ = msg.chat_id_,
                    message_ids_ = lk_lkid
                }, function(arg, data)
                    new = 0
                    lk_lkid2 = {}
                    for i = 0, data.total_count_ do
                        if data.messages_[i] and data.messages_[i].content_ and data.messages_[i].content_.ID ~=
                            "MessageText" then
            ...
            elseif text and
                (text == "تنظيف جميع الرسائل المعدله" or text ==
                    "تنظيف الرسائل المعدلة" or text == "تنظيف الرسائل المعدله") and
                Admin(msg) then
                lk_lkid = {
                    [0] = msg.id_
                }
            ...
            elseif text == 'ايدي' and tonumber(msg.reply_to_message_id_) > 0 and
                not redis:get(bot_id .. 'Status:Lock:Id:Photo' .. msg.chat_id_) then
                function Function_Status(extra, result, success)
                    tdcli_function({
                        ID = "GetUser",
                        ...
                        local Id = data.id_
                        local NumMsgg1 = redis:get(bot_id .. 'Num:Message:Userr' .. msg.chat_id_ .. ':' .. data.id_) or
                                             0
                ...
                return false
            elseif text and text:match("^ايدي @(.*)$") and
                not redis:get(bot_id .. 'Status:Lock:Id:Photo' .. msg.chat_id_) then
                ...
                function Function_Status(extra, result, success)
                    if result.id_ then
                        tdcli_function({
                            ...
                            local Id = data.id_
                            local NumMsgg1 =
                                redis:get(bot_id .. 'Num:Message:Userr' .. msg.chat_id_ .. ':' .. data.id_) or 0
                            local nummsg = redis:get(bot_id .. 'Num:Message:User' .. msg.chat_id_ .. ':' .. data.id_) or
                                               1
                            local NumMsg = (NumMsgg1 + nummsg)
                            local TotalMsg = Total_message(NumMsg)
                            local Status_Gps = Get_Rank(Id, msg.chat_id_)
                            local NumMessageEdit =
                                redis:get(bot_id .. 'Num:Message:Edit' .. msg.chat_id_ .. data.id_) or 0
                            local Num_Games =
                                redis:get(bot_id .. "Status:Msg_User" .. msg.chat_id_ .. ":" .. data.id_) or 0
                            local Add_Mem = redis:get(bot_id .. "Num:Add:Memp" .. msg.chat_id_ .. ":" .. data.id_) or 0
                            send(msg.chat_id_, msg.id_,
                                '\n*⌑︙ايديه ← ' .. Id .. '\n⌑︙رسائله ← ' .. NumMsg ..
                                    '\n⌑︙معرفه ← *[' .. UserName_User .. ']*\n⌑︙تفاعله ← ' ..
            ...
            elseif text == "الاسرع" or tect == "ترتيب" then
                if redis:get(bot_id .. "Status:Lock:Game:Group" .. msg.chat_id_) then
                    ...
                    KlamSpeed = {"سحور", "سياره", "استقبال", "قنفه", "ايفون", "بزونه",
                                 "مطبخ", "كرستيانو", "دجاجه", "مدرسه", "الوان", "غرفه",
                                 "ثلاجه", "كهوه", "سفينه", "العراق", "محطه", "طياره",
                                 "رادار", "منزل", "مستشفى", "كهرباء", "تفاحه", "اخطبوط",
                                 "سلمون", "فرنسا", "برتقاله", "تفاح", "مطرقه", "بتيته",
                                 "لهانه", "شباك", "باص", "سمكه", "ذباب", "تلفاز", "حاسوب",
                                 "انترنيت", "ساحه", "جسر"};
            ...
            elseif text == "معاني" then
                if redis:get(bot_id .. "Status:Lock:Game:Group" .. msg.chat_id_) then
                    ...
                    Maany_Rand = {"قرد", "دجاجه", "بطريق", "ضفدع", "بومه", "نحله", "ديك",
                                  "جمل", "بقره", "دولفين", "تمساح", "قرش", "نمر",
                                  "اخطبوط", "سمكه", "خفاش", "اسد", "فأر", "ذئب", "فراشه",
                                  "عقرب", "زرافه", "قنفذ", "تفاحه", "باذنجان"}
                    ...
                    send(msg.chat_id_, msg.id_,
                        "⌑︙اسرع واحد يدز معنى السمايل ~ {" .. name .. "}")
                    return false
                end
...
⌑︙الفائز يحصل على { 3 } من النقاط *
            ...
            elseif text == "امثله" then
                if redis:get(bot_id .. "Status:Lock:Game:Group" .. msg.chat_id_) then
                    mthal = {"جوز", "ضراطه", "الحبل", "الحافي", "شقره", "بيدك", "سلايه",
                             "النخله", "الخيل", "حداد", "المبلل", "يركص", "قرد",
                             "العنب", "العمه", "الخبز", "بالحصاد", "شهر", "شكه",
                             "يكحله"};
...
𓂅 . [Source Info ](t.me/planther0)     

𓂅 . [planther iNDT](t.me/planther0)     
 
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
𓂅 . [TWS planther](t.me/ubuuuBoT)     
]])
...
⌑︙قناة البوت ←* @ufuuu
            ...
            elseif text == 'م5' and porgbot(msg) then
                send(msg.chat_id_, msg.id_, [[*
...
*]])
            elseif text == 'رسائلي' then
                local NumMsgg1 =
                    redis:get(bot_id .. 'Num:Message:Userr' .. msg.chat_id_ .. ':' .. msg.sender_user_id_) or 0
                local nummsg = redis:get(bot_id .. 'Num:Message:User' .. msg.chat_id_ .. ':' .. msg.sender_user_id_) or
                                   1
                local NumMsg = (NumMsgg1 + nummsg)
            ...
            elseif text and text:match("^بيع مجوهراتي (%d+)$") then
                ...
                if tonumber(NUMPY) == tonumber(0) then
                    ...
                    return false
                elseif tonumber(redis:get(bot_id .. "Num:Add:Games" .. msg.chat_id_ .. msg.sender_user_id_)) ==
                    tonumber(0) then
                    send(msg.chat_id_, msg.id_,
                        "⌑︙ليس لديك جواهر من الالعاب \n⌑︙اذا كنت تريد ربح الجواهر \n⌑︙ارسل الالعاب وابدأ اللعب ! ")
                else
                ...
                return false
            elseif text and text:match("^اضف رسائل (%d+)$") and msg.reply_to_message_id_ == 0 and
                Constructor(msg) then
                ...
                return false
            elseif text and text:match("^اضف مجوهرات (%d+)$") and msg.reply_to_message_id_ == 0 and
                Constructor(msg) then
            ...
            elseif text and text:match("^اضف مجوهرات (%d+)$") and msg.reply_to_message_id_ ~= 0 and
                ...
                function reply(extra, result, success)
                    redis:incrby(bot_id .. "Num:Add:Games" .. msg.chat_id_ .. result.sender_user_id_,
                        text:match("^اضف مجوهرات (%d+)$"))
                    send(msg.chat_id_, msg.id_, "⌑︙تم اضافه عدد مجوهرات : " ..
                        text:match("^اضف مجوهرات (%d+)$") .. " ")
                end
                tdcli_function({
                    ID = "GetMessage",
            ...
            elseif text and text:match("^اضف رسائل (%d+)$") and msg.reply_to_message_id_ ~= 0 and
                ...
                function reply(extra, result, success)
                    ...
                    redis:incrby(bot_id .. "Num:Message:Userr" .. msg.chat_id_ .. ":" .. result.sender_user_id_,
                        text:match("^اضف رسائل (%d+)$"))
                    send(msg.chat_id_, msg.id_, "⌑︙تم اضافه عدد الرسائل : " ..
                        text:match("^اضف رسائل (%d+)$") .. " ")
                end
                tdcli_function({
                    ID = "GetMessage",
                ...
                return false
            elseif text == "تنظيف المشتركين" and Dev_planther(msg) then
                local pv = redis:smembers(bot_id .. 'Num:User:Pv')
                local sendok = 0
                for i = 1, #pv do
                    tdcli_function({
                ...
                return false
            elseif text == "تنظيف الكروبات" and Dev_planther(msg) then
                local group = redis:smembers(bot_id .. 'ChekBotAdd')
                local w = 0
                local q = 0
                for i = 1, #group do
            ...
            elseif text == "اطردني" or text == "طردني" then
                if not redis:get(bot_id .. "Status:Cheking:Kick:Me:Group" .. msg.chat_id_) then
                    ...
                    tdcli_function({
                        ...
                        if (data and data.code_ and data.code_ == 400 and data.message_ == "CHAT_ADMIN_REQUIRED") then
                            send(msg.chat_id_, msg.id_,
                                "⌑︙ ليس لدي صلاحية حظر المستخدمين يرجى تفعيلها !")
                            return false
                        end
                        ...
                        if data and data.code_ and data.code_ == 400 and data.message_ == "USER_ADMIN_INVALID" then
                            send(msg.chat_id_, msg.id_,
                                "⌑︙ عذرا لا استطيع طرد ادمنية المجموعه")
                            return false
                        end
                    ...
                    send(msg.chat_id_, msg.id_,
                        "⌑︙ امر اطردني تم تعطيله من قبل المدراء ")
                end
            ...
            elseif text == ("رفع الادمنيه") and Owner(msg) then
                tdcli_function({
                    ...
                    if num2 == 0 then
                        ...
                        send(msg.chat_id_, msg.id_,
                            "⌑︙ تمت ترقية - " .. num2 .. " من ادمنية المجموعه")
                    end
                end, nil)
                    ...
                    for i = 0, #admins do
                        if data.members_[i].status_.ID == "ChatMemberStatusCreator" then
                            ...
                            tdcli_function({
                                ...
                                local UserName = (b.username_ or "ufuuu")
            ...
            elseif text == ("رفع المنشئ") and porgbot(msg) then
                tdcli_function({
                    ID = "GetChannelMembers",
                    channel_id_ = msg.chat_id_:gsub("-100", ""),
                    filter_ = {
            ...
            elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_planther(msg) then
                redis:set(bot_id .. 'Num:Add:Bot', text:match("تعين عدد الاعضاء (%d+)$"))
                send(msg.chat_id_, msg.id_,
                    '*⌑︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : ' ..
                        text:match("تعين عدد الاعضاء (%d+)$") .. ' عضو *')
            elseif text == 'الاحصائيات' and porgbot(msg) then
                send(msg.chat_id_, msg.id_,
                    '*⌑︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌑︙عدد المجموعات : ' ..
                        (redis:scard(bot_id .. 'ChekBotAdd') or 0) .. '\n⌑︙عدد المشتركين : ' ..
                        (redis:scard(bot_id .. 'Num:User:Pv') or 0) .. '*')
            elseif text == 'المطور' or text == 'مطور' then
                local TextingDevplanther = redis:get(bot_id .. 'Texting:Devplanther')
                if TextingDevplanther then
                    send(msg.chat_id_, msg.id_, TextingDevplanther)
                else
                    send(msg.chat_id_, msg.id_, '[' .. UserName_Dev .. ']')
                end
            elseif text == 'حذف كليشه المطور' and Dev_planther(msg) then
                redis:del(bot_id .. 'Texting:Devplanther')
                send(msg.chat_id_, msg.id_, '⌑︙ تم حذف كليشه المطور')
            end
        end
        -- web site : https://planther-ash.com .
        if text == 'تفعيل' and porgbot(msg) then
            if AddChannel(msg.sender_user_id_) == false then
                send(msg.chat_id_, msg.id_,
                    '\n⌑︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌑︙قناة البوت 📁.\n- @ufuuu')
                return false
            ...
            if TypeForChat ~= 'ForSuppur' then
                send(msg.chat_id_, msg.id_,
                    '🚸¦ يجب تحويل المجموعة الى خارقة. او وضع معرف للمجموعه لتصبح عامه ثم ارفع البوت وارسل تفعيل⚙️')
                return false
            end
            if msg.can_be_deleted_ == false then
                send(msg.chat_id_, msg.id_, '⌑︙البوت ليس ادمن يرجى ترقيتي !')
            ...
            tdcli_function({
                ...
                if tonumber(data.member_count_) < tonumber(redis:get(bot_id .. 'Num:Add:Bot') or 0) and
                    not Dev_planther(msg) then
                    send(msg.chat_id_, msg.id_,
                        '⌑︙لا تستطيع تفعيل المجموعه بسبب قلة عدد اعضاء المجموعه يجب ان يكون اكثر من *:' ..
                            (redis:get(bot_id .. 'Num:Add:Bot') or 0) .. '* عضو')
                    return false
                end
                tdcli_function({
                    ID = "GetUser",
        ...
        -- web site : https://planther-ash.com .
        if text == 'تعطيل' and porgbot(msg) then
            if AddChannel(msg.sender_user_id_) == false then
                send(msg.chat_id_, msg.id_,
                    '\n⌑︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌑︙قناة البوت 📁.\n- @ufuuu')
                return false
        ...
        -- web site : https://planther-ash.com .
        if text == 'تفعيل' and not porgbot(msg) and not redis:get(bot_id .. 'Free:Bot') then
            if AddChannel(msg.sender_user_id_) == false then
                send(msg.chat_id_, msg.id_,
                    '\n⌑︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌑︙قناة البوت 📁.\n- @ufuuu')
                return false
                ...
                tdcli_function({
                    ...
                    tdcli_function({
                        ...
                        tdcli_function({
                            ...
                            chat_id_ = msg.chat_id_,
                            user_id_ = msg.sender_user_id_
                        }, function(arg, DataChat)
                            if DataChat and DataChat.status_.ID == "ChatMemberStatusEditor" or DataChat and
                                DataChat.status_.ID == "ChatMemberStatusCreator" then
...
function tdcli_update_callback(data)
    ...
    elseif data.ID == ("UpdateNewMessage") then
        ...
        if (data.message_.content_.text_) then
            if (ban_Lkid["ban" .. msg.chat_id_ .. msg.sender_user_id_] and
                ban_Lkid["ban" .. msg.chat_id_ .. msg.sender_user_id_] == data.message_.content_.text_ and
                ban_Lkid["ban:count:" .. msg.chat_id_ .. msg.sender_user_id_] and
                ban_Lkid["ban:count:" .. msg.chat_id_ .. msg.sender_user_id_] >= 3) then
                ...
                return false
            elseif (ban_Lkid["ban" .. msg.chat_id_ .. msg.sender_user_id_] and
                ban_Lkid["ban" .. msg.chat_id_ .. msg.sender_user_id_] == data.message_.content_.text_ and
                ban_Lkid["ban:count:" .. msg.chat_id_ .. msg.sender_user_id_]) then
                ban_Lkid["ban:count:" .. msg.chat_id_ .. msg.sender_user_id_] =
                    ban_Lkid["ban:count:" .. msg.chat_id_ .. msg.sender_user_id_] + 1;
            elseif (ban_Lkid["ban" .. msg.chat_id_ .. msg.sender_user_id_] and
                ban_Lkid["ban" .. msg.chat_id_ .. msg.sender_user_id_] == data.message_.content_.text_) then
                ban_Lkid["ban:count:" .. msg.chat_id_ .. msg.sender_user_id_] = 1;
            else
        ...
        if msg.content_.ID == "MessageChatAddMembers" then
            redis:set(bot_id .. "Who:Added:Me" .. msg.chat_id_ .. ":" .. msg.content_.members_[0].id_,
                msg.sender_user_id_)
            local mem_id = msg.content_.members_
                ...
                if msg.content_.members_[i].type_.ID == "UserTypeBot" and not Admin(msg) and Bots == "kick" then
                    ...
                    if Json_Info.ok == true and #mem_id == i then
                        ...
                        tdcli_function({
                            ...
                            local admins = tah.members_
                            for i = 0, #admins do
                                if tah.members_[i].status_.ID ~= "ChatMemberStatusEditor" and not is_Admin(msg) then
                                    tdcli_function({
                                        ID = "ChangeChatMemberStatus",
                                        chat_id_ = msg.chat_id_,
                                        user_id_ = admins[i].user_id_,
        ...
        if tonumber(msg.sender_user_id_) ~= tonumber(bot_id) then
            if msg.sender_user_id_ and RemovalUserGroup(msg.chat_id_, msg.sender_user_id_) then
                ...
                return false
            elseif msg.content_ and msg.content_.members_ and msg.content_.members_[0] and msg.content_.members_[0].id_ and
                RemovalUserGroup(msg.chat_id_, msg.content_.members_[0].id_) then
            ...
            elseif msg.sender_user_id_ and RemovalUserGroups(msg.sender_user_id_) then
                ...
                return false
            elseif msg.content_ and msg.content_.members_ and msg.content_.members_[0] and msg.content_.members_[0].id_ and
                RemovalUserGroups(msg.content_.members_[0].id_) then
                KickGroup(msg.chat_id_, msg.content_.members_[0].id_)
                Delete_Message(msg.chat_id_, {
                    [0] = msg.id_
                })
    ...
    elseif data.ID == ("UpdateMessageEdited") then
        tdcli_function({
            ...
            message_id_ = tonumber(data.message_id_)
        }, function(extra, result, success)
            local textedit = result.content_.text_
            redis:incr(bot_id .. 'Num:Message:Edit' .. result.chat_id_ .. result.sender_user_id_)
            if redis:get(bot_id .. "Status:Lock:edit" .. result.chat_id_) and not textedit and
                not PresidentGroup(result) then
                Delete_Message(result.chat_id_, {
                    [0] = data.message_id_
                })
            ...
            if not Vips(result) then
                ------------------------------------------------------------------------
                if textedit and textedit:match("[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]") or textedit and
                    textedit:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or textedit and
                    textedit:match("[Tt].[Mm][Ee]") or textedit and textedit:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
                    textedit and textedit:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
                ...
                elseif textedit and textedit:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or textedit and
                    textedit:match("[Tt].[Mm][Ee]") or textedit and textedit:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
                    textedit and textedit:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
                    if redis:get(bot_id .. "Status:Lock:Link" .. msg.chat_id_) then
                        Delete_Message(result.chat_id_, {
                ...
                elseif textedit and textedit:match("[hH][tT][tT][pP][sT]") or textedit and
                    textedit:match("[tT][eE][lL][eE][gG][rR][aA].[Pp][Hh]") or textedit and
                    textedit:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa].[Pp][Hh]") then
                    if redis:get(bot_id .. "Status:Lock:Link" .. msg.chat_id_) then
                        Delete_Message(result.chat_id_, {
                            [0] = data.message_id_
                        })
